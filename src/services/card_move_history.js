const cardMoveHistoryService = ({
  ...
  const createCardMoveHistoryifNotExist = async (inputs) => {
    ...
    (!!response === true
      && data.listBefore.id === response.listFromId
      && data.listAfter.id === response.ListToId) === false
      && createHistory({
        cardId: data.card.id,
        listFromId: data.listBefore.id,
        ListToId: data.listAfter.id,
        memberId: action.memberCreator.id,
      });
    return true;
  };

  const serviceGetCardMoveHistory = async ({
    listFromId,
    ListToId,
    startDate,
    endDate,
  }) => getCardMoveHistory(listFromId, ListToId, startDate, endDate);
