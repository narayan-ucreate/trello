const cardService = ({
  ...
  const attachMemberWithSpecificCard = async (inputs) => {
    ...
    !!(await checkMemberAlreadyAttachedOrNot(memberId, cardId)) === false
      && (await attachMemberToCard({ cardId, memberId }));
    return true;
  };
  ...
  const attachLabelWithSpecificCard = async (inputs) => {
    ...
    !!(await checkLabelAlreadyAttachedOrNot(cardId, labelId)) === false
      && (await attachLabelToCard({ cardId, labelId }));
    return true;
  };
  return {
    createCardfNotExist,
